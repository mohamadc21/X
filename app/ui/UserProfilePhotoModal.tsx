      ...
      <ModalContent>
        {mode === 'header' ? (
          <div className="fixed top-1/2 -translate-y-1/2 left-0 right-0 flex items-center justify-center h-[400px]">
            <Image src={userInfo?.header_photo!} alt={userInfo?.name!} fill />
          </div>
        ) : (
          <ModalBody className="flex items-center justify-center">
            <Image src={userInfo?.profile!} objectFit="cover" alt={userInfo?.name!} width={400} height={400} className="rounded-full" />
          </ModalBody>
        )}
      </ModalContent>
    </Modal>
